...
def mapFilesDictionary(validator):
    ...
    for file in range(len(input_filepaths)):
        ...
        currFile_Index = {filename[-1]: idxList}
        mapFiles_Index.update(currFile_Index)
...
def add(validator, input_value_list, idx, cnt):
    ...
    # input_value_list.insert(idx+cnt, [val])
...
def countCompare(validator, input_value_list, idx, cnt):
    ...
    # output_cmp_with = validator["count_output_cmp_with"]
...
def idValueMatch(data, schema):
    ...
    # print(output_field, ": ", data, val)
    # if(output_field == "cost"):
...
fastavro.write.LOGICAL_WRITERS["string-snapshot_id"] = idValueMatch
fastavro.read.LOGICAL_READERS["string-snapshot_id"] = decode
...
fastavro.write.LOGICAL_WRITERS["boolean-has_snapshot_chain"] = idValueMatch
fastavro.read.LOGICAL_READERS["boolean-has_snapshot_chain"] = decode

fastavro.write.LOGICAL_WRITERS["boolean-is_adhoc"] = idValueMatch
fastavro.read.LOGICAL_READERS["boolean-is_adhoc"] = decode

fastavro.write.LOGICAL_WRITERS["boolean-is_online"] = idValueMatch
fastavro.read.LOGICAL_READERS["boolean-is_online"] = decode

fastavro.write.LOGICAL_WRITERS["double-size"] = idValueMatch
fastavro.read.LOGICAL_READERS["double-size"] = decode
...
for record in output_json:
    ...
    except (ValueError, TypeError, NotFoundError, MapError, KeyError, IndexError, ZeroDivisionError, IOError) as err:
        logger.error(err)
    i = i + 1
